      ...
      <ul class="list" v-show="total">
          <li class="item" v-for="item in list" :key="item.id">
            <a :href="item.url" target="_blank">
              <iframe :src="item.url"  class="poster" frameborder="0" scrolling="no"></iframe>
  ...
  export default {
    ...
    methods: {
      ...
      deleteMaterial(params){
        MaterialDeleteApi(params).then(res=>{
          ...
          if(res.errcode===1){
            ...
            var params = {token:token,pageIndex:1,pageSize:16,type:'video'};
      ...
      handleCurrentChange(currentPage){
        let token = localStorage.getItem("accessToken");
        var params = {token:token,pageIndex:currentPage,pageSize:16,type:'video'};
        this.getMaterialList(params);
      },
      ...
      deleteHandle(id) {
        this.$confirm('您确定要删除该项数据吗?', '美编', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
          let token = localStorage.getItem('accessToken');
          var params = {token:token,type:'video',ids:[id]};
      ...
      multipleDelete(){
        ...
        if(!len){
          ...
          this.$confirm('您确定要删除该项数据吗?', '美编', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
            ...
            let token = localStorage.getItem('accessToken');
            var params = {token:token,type:'video',ids:ids};
            this.deleteMaterial(params);
          }).then(()=>{
            console.log('执行了取消删除操作');
          })
    ...
    mounted(){
      let token = localStorage.getItem("accessToken");
      var params = {token:token,pageIndex:this.currentPage,pageSize:16,type:'video'};
      this.getMaterialList(params);
    }
  }
</script>
